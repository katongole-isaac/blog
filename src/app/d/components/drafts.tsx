...
import { type IBlogTable } from "./published";
...
import { clearBlogDeletingError, deleteDrafts, fetchDraftBlogs, getBlogDeletingState, getDraftBlogState, saveActiveDraft } from "@/store/blogSlice";
import { deleteDraftEntry } from "@/store/editorSlice";
...
const DraftBlogs = () => {
  ...
  const { data: blogs, isLoading } = useAppSelector(getDraftBlogState);
  ...
  const mappedData: IBlogTable[] = blogs
    ? blogs.map((blog: any) => {
        const blogURL = `/d/create?draft=${blog?.filename}`;
  ...
  const handleBlogDelete = () => {
    ...
    dispatch(deleteDrafts([url]));
    dispatch(deleteDraftEntry(singleBlogToDelete?.filename!));

    setShowDeleteAlert({ all: false, single: false });
  };

  const handleBlogEdit = (blog: IBlogTable) => {
    const draftEditURL = `/d/create?draft=${blog?.filename}`;
    const { url, filename, downloadUrl, pathname } = blog;
    dispatch(saveActiveDraft({ data: { filename: filename!, url, downloadUrl, pathname } }));
    router.push(draftEditURL);
  };
  ...
  const columns: ITableColum[] = [
    ...
    },
    {
      ...
      content: (item) =>
        item ? (
          <Link className="text-blue-500 line-clamp-1 text-ellipsis pl-0" href={item.blogURL}>
            {item.title}
          </Link>
        ) : null,
    },
